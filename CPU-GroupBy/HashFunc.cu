...
__constant__ uint32_t  MurmurHash3_x64_32_tab[256] = {
    ...
    const size_t current_byte)
{
  ...
  return (key_columns[column_idx*num_key_rows + idx] >> shift) & 0xff;
}
    ...
    uint32_t MurmurHash3_x64_32_result[], uint32_t seed)
{
    ...
    if (idx < num_key_rows) {
        ...
        for (size_t i = 0; i < Message_size; ++i) {
            uint8_t message = retrieve_byte<T>(key_columns, num_key_columns,num_key_rows, idx, i);
            hash = MurmurHash3_x64_32_tab[(hash ^ message)] ^ (hash >> 16);
        }
        MurmurHash3_x64_32_result[idx] ^= hash;
    }
}
